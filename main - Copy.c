			...
			printf("2y ");
			  ...
			  printf("1x ");
				  if (F==1)  // 1. Problem
                     ...
                     printf("2x ");
				  /*
				  FrmResizeimage:=TFrmResizeimage.Create(self);

FrmResizeimage.PnlProperties.Visible   := False ;
FrmResizeimage.PnlAgent.Visible        := True ;
FrmResizeimage.PnlProject.Visible      := False ;
FrmResizeimage.PnlProjectimage.Visible := False ;

FrmResizeimage.AgentNo := inttostr(TxtAName.Tag) ;
FrmResizeimage.ShowModal;
FrmResizeimage.Free;
...
AdvResim.ClearRows(1,AdvResim.Rowcount-1);

//ftp.Host := 'ftp.cyprusnorthernproperty.com';
//ftp.Username := 'mustafa@cyprusnorthernproperty.com';
//ftp.password := 'b2!HdnJ^W';
//ftp.Connect;

///////////////////////////////////////////////////
qry1.close;
qry1.sql.clear;
qry1.sql.add('Select image from agents where no=:no ');
Qry1.Parameters.ParamByName('no').Value := inttostr(TxtAName.Tag) ;
Qry1.Prepared := True;
qry1.open;


try
ftp.Get('./uploads/agent/'+qry1.fieldbyname('image').asstring, './resimler/'+qry1.fieldbyname('image').asstring,True);
ImgAgent.Picture.LoadFromFile('./resimler/'+qry1.fieldbyname('image').asstring);
except
